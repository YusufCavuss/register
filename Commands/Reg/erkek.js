...
let embed = new Discord.MessageEmbed()
    .setDescription(`<@!${adam.user.id}> adlı kullanıcıyı erkek olarak kayıt edildi!`)
...
adam.roles.add(ayarlar.Rol.Erkek).catch(e => {message.channel.send(`**Hata var!** \nSebebi: **${e}**`)});
...
log.send(embed).catch(e => {message.channel.send(`**Hata var!** \nSebebi: **${e}**`)});
...
let newStaffData = new Register({
    ...
    womanRegister: 0, manRegister: 1, userNames: []
    }).save()} else {
...
if(!registerData){
    ...
    userNames: [{ nick: `${isim.charAt(0).toUpperCase() + isim.slice(1).toLowerCase()}・${yas}`, type: `Erkek`}]
    }).save()} else {
    registerData.userNames.push({ nick: `${isim.charAt(0).toUpperCase() + isim.slice(1).toLowerCase()}・${yas}`, type: `Erkek`})
    registerData.save()};
